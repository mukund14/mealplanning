# Pantry → Plate (AI‑Assisted) — Streamlit App
# -------------------------------------------------
# Human-written, boring-on-purpose. Rules first for reliability,
# Chef AI for creativity, plus simple analytics you can explain.
#
# Change log (debug fix): removed matplotlib (not available) and replaced
# charts with Streamlit-native charts. Also fixed multiline strings.
...
# Optional OpenAI — only used if toggled on with an API key.
try:
...
st.set_page_config(page_title="Pantry → Plate (AI‑Assisted)", page_icon="🍳", layout="wide")
...
class Recipe:
    ...
    def matches(
        ...
        if cuisine_filter and not (set(map(norm, self.cuisines)) & cuisine_filter):
            return False, {}
        # Diet filter — if you selected diet tags, the recipe must include all of them
    ...
    def score(self, have_norm: Set[str], craving_weights: Dict[str, int]) -> int:
        # Required/optional overlap + time bonus + tiny flavor bias
...
# -------------------------------------------------
# Smart combos (dynamic)
# -------------------------------------------------

def smart_combo_recipes(have_ings: Set[str], have_apps: Set[str]) -> List[Recipe]:
    combos: List[Recipe] = []

    # Tortillas + beans → quesadilla; masala riff if spices exist
    if {"tortillas", "beans (canned)"} <= have_ings and (
        "stovetop (pan/pot)" in have_apps or "toaster / toaster oven" in have_apps
    ):
        combos.append(
            Recipe(
                id="smart_quesadilla",
...
# -------------------------------------------------
# Hero: make the AI assist stand out
# -------------------------------------------------
...
recipes_all: Dict[str, Recipe] = dict(st.session_state["recipes"])  # copy
...
def ai_variations(api_key, model, enabled, recipe: Recipe, have: List[str], apps: List[str]) -> str:
...
def ai_substitutions(api_key, model, enabled, recipe: Recipe, missing: List[str], have: List[str]) -> str:
...
# ----- Tab 3: Analytics (Streamlit-native charts, no matplotlib) -------------
...
with t3:
    ...
    if rows:
        ...
        # What‑if: which single ingredient unlocks the most "Make now"?
...
with t4:
    ...
    def export_md(recipes: List[Tuple[Recipe, Dict]], servings: int) -> str:
        lines = ["# Pantry → Plate — Results\n"]
        for r, meta in recipes:
...
# -------------------------------------------------
# Developer sanity tests (non-blocking)
# -------------------------------------------------
with st.expander("🔧 Developer Checks (sanity tests)"):
    try:
        # Test 1: score increases with required overlap
        r = SEED_RECIPES[0]
        s0 = r.score(set(), {"spicy":5,"comfort":5,"healthy":5,"quick":5})
        s1 = r.score(set([norm(r.required_ingredients[0])]), {"spicy":5,"comfort":5,"healthy":5,"quick":5})
        st.write("Score overlap test:", "PASS" if s1 > s0 else "FAIL")

        # Test 2: matching respects appliances
        ...
        # Test 3: time filter blocks long recipes
...
with st.expander("About & Partners"):
    st.markdown(
        """
**Riverside.fm** — I record the podcast here. Their AI editor trims filler words and tightens the cut.  
**RSS.com** — Hosting + distribution with a **paid program** so small shows can earn (even with low download counts).  
**Sida.ai** — My research buddy for collecting sources fast.
